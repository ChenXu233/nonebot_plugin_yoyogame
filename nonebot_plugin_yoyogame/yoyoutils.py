...
class Computer(Player):
    ...
    def analyse(self):
        ...
        act_able = [
            action for action in costs.keys() if (0 < costs[action] <= self.opl.energy)
        ]
        # 己方可能需要采取的防御措施 + 可以进行的攻击
        motion = ["f" + action for action in act_able] + [
            action for action in costs.keys() if (0 < costs[action] <= self.energy)
        ]
...
class Game:
    ...
    def update(self, signal):
        ...
        if self.p1.energy < 0:
            ...
            len(self.p1.signal) == 2 and len(self.p2.signal) == 2
        ):  # 双方都是攻(或悠悠)
            if p1_level == 1 and not self.p1.atk_able:  # 是空炮，但不能空炮
                result, tip = 1, "当前状态下你无法使用空炮，游戏失败！"
            # 我悠他反弹或他反弹我悠
            elif ((not p1_level) and p2_level == 4) or (
                p1_level == 4 and (not p2_level)
            ):
                result, tip = 0, "游戏继续~"
            ...
            if (
                not p1_level or p1_level == 1 or p1_level == p2_level or p1_level == 4
            ):  # 我悠或空炮或防住
                result, tip = 0, "游戏继续~"
